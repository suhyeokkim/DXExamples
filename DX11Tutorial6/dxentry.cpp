...
ID3D11PixelShader* g_D3D11PixelShaderSolid;
...
struct Tutorial6Vertex
{
	DirectX::XMFLOAT3 position;
	DirectX::XMFLOAT3 normal;
};

struct Tutorial6ConstantBuffer
{
	DirectX::XMMATRIX world;
	DirectX::XMMATRIX view;
	DirectX::XMMATRIX projection;
	DirectX::XMFLOAT4 lightDirs[2];
	DirectX::XMFLOAT4 lightColors[2];
...
HRESULT DXShaderResourceInit(const wchar_t* shaderFileName, bool debug)
{
	...
	hr = CompileShaderFromFile(shaderFileName, "pixelSolid", "ps_5_0", debug, &PSBlob);
	FAILED_MESSAGE_RETURN(hr, L"fail to compile pixel shader source..");

	hr = g_D3D11Device->CreatePixelShader(
		PSBlob->GetBufferPointer(), PSBlob->GetBufferSize(), nullptr, &g_D3D11PixelShaderSolid
	);
	PSBlob->Release();
	FAILED_MESSAGE_RETURN(hr, L"fail to create pixel shader..");

	g_VertexCount = 24;
	g_VertexSize = sizeof(Tutorial6Vertex);
	Tutorial6Vertex vertices[] =
	{
	...
	hr = CreateConstantBufferInline(g_D3D11Device, &g_D3D11ConstantBuffer, sizeof(Tutorial6ConstantBuffer));
	FAILED_MESSAGE_RETURN(hr, L"fail to create constant buffer..");
...
HRESULT DXEntryInit(HINSTANCE hInstance, HWND hWnd, UINT width, UINT height, UINT maxFrameRate, bool debug)
{
	...
	hr = DXShaderResourceInit(L"./Tutorial6.hlsl", debug);
...
void DXEntryClean()
{
	...
	if (g_D3D11PixelShaderSolid) g_D3D11PixelShaderSolid->Release();
...
void DrawCube(DirectX::XMMATRIX worldMatrix, ID3D11PixelShader* pixelShader)
{
	...
	g_D3D11ImmediateContext->PSSetConstantBuffers(0, 1, &g_D3D11ConstantBuffer);
...
void DXEntryFrameUpdate()
{
	...
	{
		Tutorial6ConstantBuffer constantBuffer;
		memset(&constantBuffer, 0, sizeof(Tutorial6ConstantBuffer));
		...
		XMMATRIX rotation = XMMatrixRotationY(-2.0f * time);
		XMVECTOR lightDir = XMLoadFloat4(&lightDirs[1]);
		lightDir = XMVector3Transform(lightDir, rotation);
		...
		constantBuffer.lightDirs[0] = lightDirs[0];
		constantBuffer.lightDirs[1] = lightDirs[1];
		constantBuffer.lightColors[0] = lightColors[0];
		constantBuffer.lightColors[1] = lightColors[1];
		constantBuffer.outputColor = XMFLOAT4(0, 0, 0, 0);
		g_D3D11ImmediateContext->UpdateSubresource(g_D3D11ConstantBuffer, 0, nullptr, &constantBuffer, 0, 0);

		DrawCube(g_World, g_D3D11PixelShader);

		for (int i = 0; i < 2; i++)
		{
			XMMATRIX lightMatrix = XMMatrixTranslationFromVector(5.0f * XMLoadFloat4(&lightDirs[i]));
			XMMATRIX lightScaleMatrix = XMMatrixScaling(0.2f, 0.2f, 0.2f);
			lightMatrix = lightScaleMatrix * lightMatrix;

			constantBuffer.world = lightMatrix;
			constantBuffer.outputColor = lightColors[i];
			g_D3D11ImmediateContext->UpdateSubresource(g_D3D11ConstantBuffer, 0, nullptr, &constantBuffer, 0, 0);

			DrawCube(g_World, g_D3D11PixelShaderSolid);
		}
	}
